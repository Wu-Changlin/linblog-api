...
//当前时间
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        $lock_time = $redis->zScore('user_list', $visitor_ip); //返回有序集中key中成员member的score
        // 黑名单锁定时间 目前设置3分钟
        if ($this->now_time - $lock_time < $this->black_list_lock_time) {
            ...
            */
            $redis->zRem('user_list', $visitor_ip); //redis中zRem命令用于移除有序集合中的一个或者是多个成员，不存在的成员将被忽略，当key存在但是不是有序集合类型是，返回一个错误
        }
        ...
        if ($ip_value >= $this->max_frequency) {
            #使用有序集合
            $redis->zAdd('user_list',$this->now_time, $visitor_ip); //命令用于将一个或者是多个于是怒以及分数值加入到有序集合中
            // return 2; //调用接口频繁
            sendMSG(403, [], '调用接口频繁');
        }
