...
//午时 11:00-12:59 未时 13:00-14:59 申时 15:00-16:59 酉时 17:00-18:59 戌时 19:00-20:59 亥时 21:00-22:59
  ...
  public static  function getZodiacHour($timestamp) {
    ...
    {
        $yearData = $this->lunarInfo[$year - $this->MIN_YEAR];
        if ($year == $this->MIN_YEAR && $month <= 2) return array(1891, '正月', '初一', '辛卯', 1, 1, '兔');
    ...
    function getLunarYearName($year)
    {
        ...
        // php 8
        $lastChar = substr($year, -1, 1);
        return $sky[$lastChar] . $earth[$year % 12];
    }
   ...
   */
    ...
    {
        if ($config == 1) {
            $arr_lunar = array('01-01' => '春节', '01-15' => '元宵节', '02-02' => '二月二', '05-05' => '端午节', '07-07' => '七夕节', '08-15' => '中秋节', '09-09' => '重阳节', '12-08' => '腊八节', '12-23' => '北方小年', '12-24' => '南方小年');
        ...
        if ($jieqi) array_push($festivals, $jieqi["name2"]);
   ...
   * 获取当前时间属于哪个时辰
   @param int $time 时间戳
   */
    function getMoment($h, $i)
    {
        $d = $h;

        if ($d == 0 || $d == 2 || $d == 4 || $d == 6 || $d == 8 || $d == 10 || $d == 12 || $d == 14 || $d == 16 || $d == 18 || $d == 20 || $d == 22) {
            $z = 60 + $i;
        } else {
            $z = $i;
        }

        $dateHash = array('1' => '一', '2' => '二', '3' => '三', '4' => '四', '5' => '五', '6' => '六', '7' => '七', '8' => '八', '9' => '九', '10' => '十 ');

        return  $dateHash[ceil($z / 15)];
    }
}
